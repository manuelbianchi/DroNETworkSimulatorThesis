...
from src.experiments.experiment_nranges import N_DRONES
...
def plot_coverage_distribution(filename_format: list, n_ranges: list, metric: str,
                               alg_ritrasmission: list, seeds: list, size_mission: int):
    """ plot for varying ndrones """
    for nc in n_ranges:
        out_data = {}
        for alg_k in alg_ritrasmission:
            out_data[alg_k] = coverage_distribution(filename_format, nc, alg_k, seeds)

        fig, ((ax1, ax2, ax3), (ax4, ax5, ax6), (ax7, ax8, ax9)) = plt.subplots(3, 3)
        fig.set_size_inches(16, 10)

        fig.suptitle('Plot distribution of coverage by' + str(nc) + 'drones')
        ...
        plt.tight_layout()
        plt.savefig(metric + "_" + str(nc) + "_.png")
        # plt.show()
        plt.clf()
...
def coverage_distribution(filename_format: str, n_ranges: int,
    ...
    Y = []
    for seed in seeds:
        file_name = filename_format.format(n_ranges, seed, alg_k)
...
def mean_std_of_metric(filename_format: str, n_ranges: int,
...
def plot_nspeeds(filename_format: list, n_ranges: list, metric: str,
    ...
    for alg_k in alg_ritrasmission:
        ...
        # for each x ticks
        for nc in n_ranges:
            data_alg_k.append(mean_std_of_metric(filename_format, nc, alg_k, seeds, metric)[0])
    ...
    if (metric != "area_covered_by_drones_ratio" and metric != "time_to_create_network_of_drones"):
        ...
        plt.xlabel("Communication Range", fontsize=LABEL_SIZE)
...
if __name__ == "__main__":
    ...
    # MANDATORY
    parser.add_argument("-nc", dest='com_range', action="append", type=int,
                        help="the communication range to use in the simulataion")
    ...
    parser.add_argument("-exp_metric", dest='exp_metric', action="store", type=str, default="com_range_",
                        help="the exp metric to run, should be in [ninterval_ speed_ ndrones_] ")

    args = parser.parse_args()

    # speed
    communication_range = args.com_range
    ...
    for metric in METRICS_OF_INTEREST:
        plot_nspeeds(pattern_file, communication_range, metric, alg_exp_suffix, n_seeds, out_dir + "_" + str(exp_metric) + "_",
                     exp_metric)

    # size_mission = 3000
    plot_coverage_distribution(pattern_file,communication_range, config.SAVE_PLOT_DIR + 'coverage_distribution',
