...
def sim_setup(com_range, seed, algorithm_routing):
    ...
    model_name = "model_" + str(com_range)
...
def exp_nranges(path_filename, com_range, in_seed, out_seed, algorithm_routing):
    # ---- Experiment 1 ---- #
    # test the routing for
    # at varying k and seed values
    scores = {}
    for seed in range(in_seed, out_seed):
        print("Running " + algorithm_routing + " with", com_range, "communication_range with seed:", seed)

        simulation = sim_setup(com_range, seed, algorithm_routing)
        simulation.run()

        simulation.save_metrics(path_filename + "out__com_range_" + str(com_range) +
                                "_seed" + str(seed) + "_alg_" + algorithm_routing)
        print("Score: ", simulation.score(), algorithm_routing)
        scores[seed] = simulation.score()
        simulation.close()

    print("Communication range: ", com_range, " - Algo: ", algorithm_routing, "- Scores: ", scores)
...
if __name__ == "__main__":
    ...
    # MANDATORY
    parser.add_argument("-nc", dest='com_range', action="store", type=int,
    ...
    exp_nranges(path_filename,communication_range, initial_seed, end_seed, algorithm_routing)
    print("Sim completed")
