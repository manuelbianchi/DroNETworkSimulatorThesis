...
def exp_nspeeds(path_filename, drone_speed, in_seed, out_seed, algorithm_routing):
    # ---- Experiment 1 ---- #
    # test the routing for
    # at varying k and seed values
    scores = {}
    for seed in range(in_seed, out_seed):
        print("Running " + algorithm_routing + " with", drone_speed, "speed with seed:", seed)

        simulation = sim_setup(drone_speed, seed, algorithm_routing)
        simulation.run()

        simulation.save_metrics(path_filename + "out__drone_speed_" + str(drone_speed) +
                                "_seed" + str(seed) + "_alg_" + algorithm_routing)
        print("Score: ", simulation.score(), algorithm_routing)
        scores[seed] = simulation.score()
        simulation.close()

    print("Drone speed: ", drone_speed, " - Algo: ", algorithm_routing, "- Scores: ", scores)
...
if __name__ == "__main__":
    ...
    # MANDATORY
    parser.add_argument("-ns", dest='drone_speed', action="store", type=int,
    ...
    exp_nspeeds(path_filename,drone_speed, initial_seed, end_seed, algorithm_routing)
    print("Sim completed")